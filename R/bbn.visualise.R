...
#' bbn.visualise(
#'   bbn.model = my_BBN, priors1 = combined,
...
bbn.visualise <- function(bbn.model, priors1, timesteps = 5, disturbance = 1, threshold = 0.2, font.size = 0.7, arrow.size = 4) {
  ...
  for (x.rep in 2:rep) {
    ...
    for (f in 1:number.nodes) {
      ...
      if (node[f, 1] > 0.5 & post.node1.try2 > post.node1) {
  ...
  if (number.nodes <= 9) {
    temp.array <- paste("s0", temp.array[1:number.nodes], sep = "")
  }
  ...
  for (x.rep in 1:rep) {
    ...
    print(plot(net, edge.arrow.size = 0.5, vertex.label = nodes$node.name, edge.curved = .3, layout = igraph::layout_on_grid))
  }
}
