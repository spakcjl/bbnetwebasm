...
bbn.predict <- function(bbn.model, ..., boot_max = 1, values = 1, figure = 1, font.size = 5) {
  ...
  for (policy in 1:policyno) {
    ...
    if (is.numeric(node[, 1]) == F) {
      stop("First column of priors values must contain numeric data")
    }
    ...
    for (aaa in 1:number.nodes) {
      node.store2[aaa, 1] <- node[aaa, 1]
    }
    ...
    for (boot in 1:boot_max) {
      ...
      while (adjust.count < adjust.number + 1) {
        ...
        if (!is.na(node.x.increase.if.node.y.increase[x_a1, y_a1])) {
          ...
          if (boot == 1) {
            node.x.increase.if.node.y.increase[x_a1, y_a1] <- node.x.increase.if.node.y.increase[x_a1, y_a1]
          } ## note if bootstrap size is set to 1, then no bootstrapping takes place
          if (boot > 1) {
      ...
      for (x.rep in 2:rep2) {
        ...
        for (f in 1:number.nodes) { ## converted via Bayes equation at this point, but testing  whether multiple nodes are pulling in the same direction - if so, combining probabilities, rather than averaging - see Stafford et al. 2015 for details
          ...
          if (node2[f, 1] > 0.5 & post.node1.try2 > post.node1) {
            post.node1 <- post.node1.try2
          }
          if (node2[f, 1] < 0.5 & post.node1.try2 < post.node1) {
            post.node1 <- post.node1.try2
          }
    ...
    p0 <- ggplot2::ggplot(data = final.output, ggplot2::aes(x = name, y = Increase)) +
      ggplot2::geom_point(stat = "identity", size = 0.5) +
      ggplot2::geom_errorbar(ggplot2::aes(ymin = LowerCI, ymax = UpperCI), width = 0.1) +
      ggplot2::xlab("") +
      ggplot2::ylab("Predicted change") +
      ggplot2::theme(text = ggplot2::element_text(size = font.size)) +
      ggplot2::scale_y_continuous(limits = c(-4, 4))


    p0 <- p0 + ggplot2::coord_flip()
  ...
  if (figure == 1) { ## plot pdf figure to working directory
    ...
    if (policyno == 1) {
      multiplot(plot.keep[[1]])
    }
    if (policyno == 2) {
      multiplot(plot.keep[[1]], plot.keep[[2]])
    }
    if (policyno == 3) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], cols = 2)
    }
    if (policyno == 4) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], plot.keep[[4]], cols = 2)
    }
    if (policyno == 5) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], plot.keep[[4]], plot.keep[[5]], cols = 2)
    }
    if (policyno == 6) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], plot.keep[[4]], plot.keep[[5]], plot.keep[[6]], cols = 2)
    }
    if (policyno == 7) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], plot.keep[[4]], plot.keep[[5]], plot.keep[[6]], plot.keep[[7]], cols = 2)
    }
    if (policyno == 8) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], plot.keep[[4]], plot.keep[[5]], plot.keep[[6]], plot.keep[[7]], plot.keep[[8]], cols = 2)
    }
    if (policyno == 9) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], plot.keep[[4]], plot.keep[[5]], plot.keep[[6]], plot.keep[[7]], plot.keep[[8]], plot.keep[[9]], cols = 2)
    }
    if (policyno == 10) {
    ...
    if (policyno == 11) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], plot.keep[[4]], plot.keep[[5]], plot.keep[[6]], plot.keep[[7]], plot.keep[[8]], plot.keep[[9]], plot.keep[[10]], plot.keep[[11]], cols = 3)
    }
    if (policyno == 12) {
      multiplot(plot.keep[[1]], plot.keep[[2]], plot.keep[[3]], plot.keep[[4]], plot.keep[[5]], plot.keep[[6]], plot.keep[[7]], plot.keep[[8]], plot.keep[[9]], plot.keep[[10]], plot.keep[[11]], plot.keep[[12]], cols = 3)
    }
