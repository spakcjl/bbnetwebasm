...
bbn.sensitivity <- function(bbn.model, boot_max = 1000, ...) {
  ...
  x <- length(list(...))
  if (x < 1) {
    stop("Need to input at least one key variable", call. = FALSE)
  }
  if (x > 3) {
    warning("Recommend a maximum of three key variables", call. = FALSE)
  }
  ...
  for (var1 in 1:max.variable) {
    ...
    for (output.param in 1:display.number) {
      ...
      if (identical(output.save, integer(0)) == T) {
        print("Key variable name cannot be found. Please, check spelling and white space")
      }
